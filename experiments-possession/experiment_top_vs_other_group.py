#!/usr/bin/env python3
"""
Top vs Other cross-validation for possession regression (CatBoost, GROUPED features).
...
Protocol per group (top, other):
  ...
  - Train on ALL remaining games (both groups), excluding the test fold.
  - Evaluate on the held-out fold.

Inputs:
- --games_root: root folder with one subfolder per game (e.g., dates)
- --games_map_json: JSON {"top": [...], "other": [...]}

Outputs (under --out_dir, default: experiment_top_vs_other_grouped):
- folds.csv, summary.csv, effective_games.json
- top_other_mae.png, top_other_rmse.png, top_other_mae_boxplot.png
- top_other_interval_mae_025_075.png
- top_other_mismatch_avg_flips.png
- all_fold_predictions.csv, top_other_mae_true_025_075.csv
- mismatch_counts_per_game.csv, mismatch_avg_flips_by_group.csv/.std.csv
- Late-window (60–90): folds_60_90.csv, summary_60_90.csv,
  top_other_mae_60_90.png, top_other_rmse_60_90.png, top_other_mae_boxplot_60_90.png,
  top_other_interval_mae_025_075_60_90.png, top_other_mismatch_avg_flips_60_90.png
"""
...
def sample_weights(y: np.ndarray, weight_scale: float = 0.8) -> np.ndarray:
    # same weighting strategy; set weight_scale=0.0 to disable
...
def train_eval_once(train_df: pd.DataFrame,
                    ...
                    seed: int,
                    return_preds: bool = True) -> Tuple[Dict[str, float], pd.DataFrame]:
    ...
    # Keep weights disabled (matches your recent scripts). Change to 0.8 to enable.
...
def run(
    games_root: str,
    games_map_json: str,
    out_dir: str = "experiment_top_vs_other_grouped",
    fold_size: int = 5,
    seed: int = 42,
    ...
    print(f"📥 Loading user group mapping from {games_map_json}…")
    ...
    for group in groups:
        ...
        if len(group_games) == 0:
            print(f"⚠️ Skipping group '{group}' (no games).")
            continue
    ...
    # Save all per-interval predictions
    if len(all_preds) == 0:
        print("ℹ️ No predictions captured; skipping extra graphs.")
        return
    all_preds_df = pd.concat(all_preds, ignore_index=True)
    all_preds_df = all_preds_df[["group","game","minute_start","minute_end","true","pred","fold_idx"]]
    ...
    # ---- Plots: Top vs Other (all minutes)
    try:
        needed = {"top", "other"}
        present = set(summary["group"].tolist())
        if needed.issubset(present):
            plot_df = summary.set_index("group").loc[["top","other"]].reset_index()

            # MAE bar
            plt.figure(figsize=(6,4))
            plt.bar(plot_df["group"], plot_df["mean_mae"], yerr=plot_df["std_mae"], capsize=6)
            plt.ylabel("Mean MAE (mistake)")
            plt.title("Top vs Other – Mean MAE (±1 std)")
            for x, y in enumerate(plot_df["mean_mae"]):
                plt.text(x, y, f"{y:.3f}", ha="center", va="bottom")
            plt.tight_layout()
            plt.savefig(out_path / "top_other_mae.png", dpi=150)
            plt.close()
            print(f"🖼️ Saved plot → {out_path/'top_other_mae.png'}")

            # RMSE bar
            plt.figure(figsize=(6,4))
            plt.bar(plot_df["group"], plot_df["mean_rmse"], yerr=plot_df["std_rmse"], capsize=6)
            plt.ylabel("Mean RMSE")
            plt.title("Top vs Other – Mean RMSE (±1 std)")
            for x, y in enumerate(plot_df["mean_rmse"]):
                plt.text(x, y, f"{y:.3f}", ha="center", va="bottom")
            plt.tight_layout()
            plt.savefig(out_path / "top_other_rmse.png", dpi=150)
            plt.close()
            print(f"🖼️ Saved plot → {out_path/'top_other_rmse.png'}")

            # Per-fold MAE boxplot
            plt.figure(figsize=(6,4))
            folds_df.boxplot(column="mae", by="group")
            plt.suptitle("")
            plt.title("Per-fold MAE by Group")
            plt.ylabel("MAE")
            plt.tight_layout()
            plt.savefig(out_path / "top_other_mae_boxplot.png", dpi=150)
            plt.close()
            print(f"🖼️ Saved plot → {out_path/'top_other_mae_boxplot.png'}")
        else:
            print("ℹ️ Skipping plots (need both 'top' and 'other').")
    except Exception as e:
        print(f"⚠️ Plotting failed: {e}")

    # Print MAE comparison
    if set(["top","other"]).issubset(set(summary['group'])):
        top_mae = float(summary.loc[summary['group']=="top","mean_mae"].iloc[0])
        other_mae = float(summary.loc[summary['group']=="other","mean_mae"].iloc[0])
        diff = top_mae - other_mae
        print("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        print("Mean MAE (mistake) by group:")
        print(f"  Top:   {top_mae:.4f}")
        print(f"  Other: {other_mae:.4f}")
        print(f"  Top − Other: {diff:+.4f}")
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

    # ===== Graph 1: MAE restricted to true in [0.25, 0.75] per group =====
    try:
        ...
        if not mid_df.empty:
            mae_mid = (mid_df
                       ...
                       .groupby("group")["abs_err"]
                       .mean()
                       .reset_index()
                       .rename(columns={"abs_err":"mae_mid"}))
            mae_mid.to_csv(out_path / "top_other_mae_true_025_075.csv", index=False)

            plt.figure(figsize=(6,4))
            plt.bar(mae_mid["group"], mae_mid["mae_mid"], capsize=6)
            for x, y in enumerate(mae_mid["mae_mid"]):
                plt.text(x, y, f"{y:.3f}", ha="center", va="bottom")
            plt.ylabel("MAE on 0.25 ≤ true ≤ 0.75")
            plt.title("Top vs Other — Mid-range MAE")
            plt.tight_layout()
            plt.savefig(out_path / "top_other_interval_mae_025_075.png", dpi=150)
            plt.close()
            print(f"🖼️ Saved plot → {out_path/'top_other_interval_mae_025_075.png'}")
        else:
            print("ℹ️ No intervals with true in [0.25, 0.75]; skipping mid-range MAE plot.")
    except Exception as e:
        print(f"⚠️ Failed to create mid-range MAE plot: {e}")

    # ===== Graph 2: AVERAGE mismatch flips per group (mean per game) =====
    try:
        ...
        # Per-game counts
        game_counts = (df.groupby(["group","game"])
        ...
        # Per-group averages (mean per game) + std for error bars
        group_avg = (game_counts.groupby("group")[["over_pred_under_true","under_pred_over_true"]]
                                .mean()
                                .reset_index()
                                .rename(columns={
        ...
        group_std = (game_counts.groupby("group")[["over_pred_under_true","under_pred_over_true"]]
                                .std()
                                .reset_index()
                                .rename(columns={
        ...
        group_avg.to_csv(out_path / "mismatch_avg_flips_by_group.csv", index=False)
        group_std.to_csv(out_path / "mismatch_avg_flips_by_group_std.csv", index=False)

        # Plot grouped bars: avg flips per game by group
        plot_df = group_avg.merge(group_std, on="group", how="left")
        wanted = ["top","other"]
        present = set(plot_df["group"])
        if set(wanted).issubset(present):
            plot_df = plot_df.set_index("group").loc[wanted].reset_index()
        ...
        plt.title("Average threshold flips per game by group")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_path / "top_other_mismatch_avg_flips.png", dpi=150)
        plt.close()
        print(f"🖼️ Saved plot → {out_path/'top_other_mismatch_avg_flips.png'}")
    except Exception as e:
        print(f"⚠️ Failed to create average flips plot: {e}")

    # ===== Late-window (minutes 60–90) versions of the 3 core graphs =====
    try:
        ...
        if late.empty:
            ...
            folds_60_90 = (late.groupby(["group","fold_idx"])
            ...
            summary_60_90 = (folds_60_90.groupby("group")
                              .agg(mean_mae=("mae","mean"),
                                   std_mae=("mae","std"),
                                   mean_rmse=("rmse","mean"),
                                   std_rmse=("rmse","std"),
                                   n_folds=("fold_idx","count"))
                              .reset_index())
            summary_60_90["std_mae"]  = summary_60_90["std_mae"].fillna(0.0)
            summary_60_90["std_rmse"] = summary_60_90["std_rmse"].fillna(0.0)
            summary_60_90.to_csv(out_path / "summary_60_90.csv", index=False)
            print(f"📊 Late-window summary → {out_path/'summary_60_90.csv'}")

            needed = {"top","other"}
            present = set(summary_60_90["group"])
            if needed.issubset(present):
                plot_df = summary_60_90.set_index("group").loc[["top","other"]].reset_index()

                # MAE (60–90)
                plt.figure(figsize=(6,4))
                plt.bar(plot_df["group"], plot_df["mean_mae"], yerr=plot_df["std_mae"], capsize=6)
                plt.ylabel("Mean MAE (mistake) – minutes 60–90")
                plt.title("Top vs Other – Mean MAE (±1 std), 60–90")
                for x, y in enumerate(plot_df["mean_mae"]):
                    plt.text(x, y, f"{y:.3f}", ha="center", va="bottom")
                plt.tight_layout()
                plt.savefig(out_path / "top_other_mae_60_90.png", dpi=150)
                plt.close()
                print(f"🖼️ Saved plot → {out_path/'top_other_mae_60_90.png'}")

                # RMSE (60–90)
                plt.figure(figsize=(6,4))
                plt.bar(plot_df["group"], plot_df["mean_rmse"], yerr=plot_df["std_rmse"], capsize=6)
                plt.ylabel("Mean RMSE – minutes 60–90")
                plt.title("Top vs Other – Mean RMSE (±1 std), 60–90")
                for x, y in enumerate(plot_df["mean_rmse"]):
                    plt.text(x, y, f"{y:.3f}", ha="center", va="bottom")
                plt.tight_layout()
                plt.savefig(out_path / "top_other_rmse_60_90.png", dpi=150)
                plt.close()
                print(f"🖼️ Saved plot → {out_path/'top_other_rmse_60_90.png'}")

                # Per-fold MAE boxplot (60–90)
                plt.figure(figsize=(6,4))
                folds_60_90.boxplot(column="mae", by="group")
                plt.suptitle("")
                plt.title("Per-fold MAE by Group (minutes 60–90)")
                plt.ylabel("MAE")
                plt.tight_layout()
                plt.savefig(out_path / "top_other_mae_boxplot_60_90.png", dpi=150)
                plt.close()
                print(f"🖼️ Saved plot → {out_path/'top_other_mae_boxplot_60_90.png'}")
            else:
                print("ℹ️ Skipping late-window plots (need both 'top' and 'other').")

            # --- Late-window Graph A: Mid-range MAE (true in [0.25, 0.75])
            ...
            if not late_mid.empty:
                mae_mid_60_90 = (late_mid
                                 ...
                                 .reset_index()
                                 .rename(columns={"abs_err": "mae_mid"}))
                mae_mid_60_90.to_csv(out_path / "top_other_mae_true_025_075_60_90.csv", index=False)

                plt.figure(figsize=(6,4))
                plt.bar(mae_mid_60_90["group"], mae_mid_60_90["mae_mid"], capsize=6)
                for x, y in enumerate(mae_mid_60_90["mae_mid"]):
                    plt.text(x, y, f"{y:.3f}", ha="center", va="bottom")
                plt.ylabel("MAE on 0.25 ≤ true ≤ 0.75 (60–90)")
                plt.title("Top vs Other — Mid-range MAE (minutes 60–90)")
                plt.tight_layout()
                plt.savefig(out_path / "top_other_interval_mae_025_075_60_90.png", dpi=150)
                plt.close()
                print(f"🖼️ Saved plot → {out_path/'top_other_interval_mae_025_075_60_90.png'}")
            else:
            ...
            game_counts_60_90 = (df_late.groupby(["group","game"])
                                   .agg(over_pred_under_true=("over_pred_under_true","sum"),
                                        under_pred_over_true=("under_pred_over_true","sum"))
                                   .reset_index())
            game_counts_60_90.to_csv(out_path / "mismatch_counts_per_game_60_90.csv", index=False)

            # Per-group averages (mean per game) + std (60–90)
            group_avg_60_90 = (game_counts_60_90.groupby("group")[["over_pred_under_true","under_pred_over_true"]]
                               .mean()
                               .reset_index()
                               .rename(columns={
                                   "over_pred_under_true": "avg_over_pred_under_true",
            ...
            group_std_60_90 = (game_counts_60_90.groupby("group")[["over_pred_under_true","under_pred_over_true"]]
                               .std()
                               .reset_index()
                               .rename(columns={
                                   "over_pred_under_true": "std_over_pred_under_true",
            ...
            group_avg_60_90.to_csv(out_path / "mismatch_avg_flips_by_group_60_90.csv", index=False)
            group_std_60_90.to_csv(out_path / "mismatch_avg_flips_by_group_60_90_std.csv", index=False)

            # Plot grouped bars: avg flips per game by group (60–90)
            plot_df = group_avg_60_90.merge(group_std_60_90, on="group", how="left")
            if set(["top","other"]).issubset(set(plot_df["group"])):  # keep order
                plot_df = plot_df.set_index("group").loc[["top","other"]].reset_index()
            ...
            plt.title("Average threshold flips per game by group (minutes 60–90)")
            plt.legend()
            plt.tight_layout()
            plt.savefig(out_path / "top_other_mismatch_avg_flips_60_90.png", dpi=150)
            plt.close()
            print(f"🖼️ Saved plot → {out_path/'top_other_mismatch_avg_flips_60_90.png'}")

    except Exception as e:
        print(f"⚠️ Failed to create late-window graphs: {e}")

    # --- MAE by true side of 0.5 (overall and 60–90) ---
    plot_mae_true_halves(all_preds_df, group_col="group",
                         out_path=out_path, prefix="top_other")
    plot_mae_true_halves(all_preds_df, group_col="group",
                         out_path=out_path, prefix="top_other",
...
if __name__ == "__main__":
    ...
    p.add_argument("--games_root", type=str, default="5min_windows",
                   help="Root folder containing one subfolder per game (e.g., dates)")
    p.add_argument("--games_map_json", type=str, required=True,
                   help="Path to JSON with {'top': [...], 'other': [...]} game folder names")
    p.add_argument("--out_dir", type=str, default="experiment_top_vs_other_grouped",
