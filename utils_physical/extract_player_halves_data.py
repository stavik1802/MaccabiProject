...
This script extracts all first_half_features.csv and second_half_features.csv files for a specific player
...
def find_player_half_features(run1_path, player_code):
    """
    Find all first_half_features.csv and second_half_features.csv files for a specific player across all games
    ...
    Returns:
        list: List of tuples (game_date, half_type, source_file_path)
    """
    player_files = []
    ...
    for game_folder in os.listdir(run1_path):
        ...
        # Look for the player's half features files
        ...
        # Check for first_half_features.csv
        first_half_path = os.path.join(player_folder_path, "first_half_features.csv")
        if os.path.exists(first_half_path):
            player_files.append((game_date, "first_half", first_half_path))
            print(f"Found first half data for {player_code} on {game_date}")
        else:
            print(f"No first half data found for {player_code} on {game_date}")
        
        # Check for second_half_features.csv
        second_half_path = os.path.join(player_folder_path, "second_half_features.csv")
        if os.path.exists(second_half_path):
            player_files.append((game_date, "second_half", second_half_path))
            print(f"Found second half data for {player_code} on {game_date}")
        else:
            print(f"No second half data found for {player_code} on {game_date}")
...
def copy_player_files(player_files, player_code, output_base_dir="player_data"):
    ...
    Args:
        player_files (list): List of tuples (game_date, half_type, source_file_path)
    ...
    for game_date, half_type, source_file in player_files:
        # Create new filename with date and half type
        new_filename = f"{half_type}_features_{game_date}.csv"
...
def process_single_player(player_code, run1_path):
    ...
    player_files = find_player_half_features(run1_path, player_code)
    
    if not player_files:
        print(f"No data found for player {player_code}")
        return False
    
    print(f"\nFound {len(player_files)} half-feature files for {player_code}")
